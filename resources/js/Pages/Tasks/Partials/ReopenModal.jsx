...
const ReopenModal = ({ isOpen, close, task }) => {
    const reopenHandler = () => {
        router.patch(
            route("task.reopen", task.number),
            {},
            {
                preserveScroll: true,
                onSuccess: () => {
                    close();
                    Toast("success", "Task successfully re-opened");
                },
            }
        );
    };
    ...
    return (
        <Modal
            title={`Re-open task - <b>${task.number}</b>`}
            isOpen={isOpen}
            close={close}
        >
            <Modal.Body className="flex flex-col gap-y-3">
                Are you sure to re-open this task?
            </Modal.Body>
