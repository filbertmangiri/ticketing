...
const Index = (props) => {
    const { data: categories, meta, queries, attributes } = props.categories;
    ...
    const reload = useCallback(
        debounce((query) => {
            ...
            router.get(route("category.index"), pickBy(query), {
                preserveState: true,
                preserveScroll: true,
            });
        }, 300),
    ...
    const confirmDeleteHandler = () => {
        router.delete(
            route(
                `category.${
                    selectedCategoryToDelete.deleted_at
                        ? "forceDelete"
                        : "destroy"
                }`,
                selectedCategoryToDelete.slug
            ),
            {
                preserveScroll: true,
                onSuccess: () => {
    ...
    const [selectedCategoryToRestore, setSelectedCategoryToRestore] = useState(
        {}
    );

    const restoreHandler = (category) => {
    ...
    const confirmRestoreHandler = () => {
        router.patch(
            route("category.restore", selectedCategoryToRestore.slug),
            {},
            {
                preserveScroll: true,
                onSuccess: () => {
    ...
    return (
        <>
            ...
            {(can("delete category") || can("force delete category")) && (
                <DeleteModal
                    isOpen={isDeleteModalOpen}
                    close={closeDeleteModal}
            ...
            {can("restore category") && (
                <RestoreModal
                    isOpen={isRestoreModalOpen}
                    close={closeRestoreModal}
                    category={selectedCategoryToRestore}
...
Index.layout = (page) => <DashboardLayout children={page} title="Categories" />;
