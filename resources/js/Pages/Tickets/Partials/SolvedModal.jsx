...
const SolvedModal = ({ isOpen, close, ticket }) => {
    const solvedHandler = () => {
        router.patch(
            route("ticket.solved", ticket.number),
            {},
            {
                preserveScroll: true,
                onSuccess: () => {
                    close();
                    Toast("success", "Ticket successfully marked as solved");
                },
            }
        );
    };
    ...
    return (
        <Modal
            title={`Ticket solved - <b>${ticket.number}</b>`}
            isOpen={isOpen}
            close={close}
        >
            <Modal.Body className="flex flex-col gap-y-3">
                Are you sure to mark this ticket as solved?
            </Modal.Body>
