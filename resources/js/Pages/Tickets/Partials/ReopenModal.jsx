...
const ReopenModal = ({ isOpen, close, ticket }) => {
    const reopenHandler = () => {
        router.patch(
            route("ticket.reopen", ticket.number),
            {},
            {
                preserveScroll: true,
                onSuccess: () => {
                    close();
                    Toast("success", "Ticket successfully re-opened");
                },
            }
        );
    };
    ...
    return (
        <Modal
            title={`Re-open ticket - <b>${ticket.number}</b>`}
            isOpen={isOpen}
            close={close}
        >
